...
| coder | 規約に従った実装 + クリーンアップ。レビュー指摘時は PR コメントを読んで修正 | `docs/ARCHITECTURE.md` + `docs/CODING_GUIDELINES.md` + PR |
...
pm (ユーザーとの窓口)
  ...
  └── tl (issue 単位)
        ...
        1. coder — 実装 + クリーンアップ → コミット + プッシュ
        2. reviewer   — PR レビュー (approve or changes requested)
           - VIOLATIONS → 1 に戻り修正
